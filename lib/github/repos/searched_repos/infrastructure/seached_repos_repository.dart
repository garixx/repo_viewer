...
class SearchedReposRepository {
  ...
  Future<Either<GithubFailure, Fresh<List<GithubRepo>>>> getSearchedReposPage(String query, int page) async {
    try {
      final remotePageItems = await _remoteService.getSearchedRepos(query, page);
      return right(
        remotePageItems.maybeWhen(
            withNewData: (data, maxPage) => Fresh.yes(data.toDomain(), isNextPageAvailable: page < maxPage),
